...
def test_box_add_multiple_fruits_to_full(small_box, apple, banana):
    small_box.add_fruit(apple)
    assert small_box.fruits == [apple]
    assert small_box.volume == apple.volume
...
def test_box_add_fruit_with_add_operator_invalid_type(small_box, big_box):
    with pytest.raises(TypeError) as exc_info:
        small_box = small_box + big_box
    assert str(exc_info.value) == "unsupported operand type(s) for +: 'Box' and 'Box'"


def test_box_add_fruit_with_iadd_operator_invalid_type(small_box, big_box):
    with pytest.raises(TypeError) as exc_info:
        small_box = small_box + big_box
    assert str(exc_info.value) == "unsupported operand type(s) for +: 'Box' and 'Box'"
